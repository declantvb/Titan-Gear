...
public class DragItem : MonoBehaviour, IDragHandler, IPointerDownHandler, IEndDragHandler
{
	...
	private void Start()
	{
		...
		var draggingItem = GameObject.FindGameObjectWithTag("DraggingItem");
		rectTransformSlot = draggingItem.GetComponent<RectTransform>();
		draggedItemBox = draggingItem.transform;
	}
	...
	public void OnEndDrag(PointerEventData data)
	{
		if (data.button == PointerEventData.InputButton.Left)
		{
			...
			if (newSlot != null)
			{
				...
				Inventory targetInventory = secondItemRectTransform.GetComponentInParent<Inventory>();

				//dragging in an Inventory
				if (targetInventory.GetComponent<Hotbar>() == null && targetInventory.GetComponent<EquipmentSystem>() == null && targetInventory.GetComponent<CraftSystem>() == null)
				{
					MoveToInventory(newSlot, firstItemGameObject, secondItemGameObject, firstItemRectTransform, secondItemRectTransform, firstItem, secondItem);
				}

				//dragging into a equipmentsystem/charactersystem
				if (targetInventory.GetComponent<EquipmentSystem>() != null)
				{
					MoveToEquipment(newSlot, firstItemGameObject, secondItemGameObject, firstItemRectTransform, secondItemRectTransform, firstItem, secondItem, targetInventory);
				}

				//dragging into crafting
				/*if (Inventory.GetComponent<CraftSystem>() != null)
				{
					...
					if (newSlotChildCount != 0 && isOnSlot)
					{
						...
						{
							var eS = GameObject.FindGameObjectWithTag("EquipmentSystem").GetComponent<EquipmentSystem>();
							...
							else if (!fitsIntoStack && rest == 0)
							{
								...
								if (oldSlot.transform.parent.parent.GetComponent<EquipmentSystem>() != null && firstItem.itemType == secondItem.itemType)
								{
									...
									//find old slot index
									var oldIndex = -1;
									for (int i = 0; i < oldSlot.transform.parent.childCount; i++)
									{
										if (oldSlot.transform.Equals(oldSlot.transform.parent.GetChild(i)))
										{
											oldIndex = i;
										}
									}
									...
									var oldPartSlot = oldIndex >= 0 ? eS.hardpointOfSlots[oldIndex] : null;
									var newPartSlot = newIndex >= 0 ? eS.hardpointOfSlots[newIndex] : null;
									oldSlot.transform.parent.parent.GetComponent<Inventory>().EquiptItem(oldPartSlot, secondItem);
									newSlot.transform.parent.parent.parent.parent.GetComponent<Inventory>().UnEquipItem1(newPartSlot, firstItem);
								}
					...
					else
					{
						...
						else
						{
							...
							if (newSlot.transform.parent.parent.GetComponent<EquipmentSystem>() == null && oldSlot.transform.parent.parent.GetComponent<EquipmentSystem>() != null)
							{
								...
								var partSlot = index >= 0 ? eS.hardpointOfSlots[index] : null;
								oldSlot.transform.parent.parent.GetComponent<Inventory>().UnEquipItem1(partSlot, firstItem);
							}
						}
					}
				}*/
	...
	private void MoveToInventory(Transform newSlot, GameObject firstItemGameObject, GameObject secondItemGameObject, RectTransform firstItemRectTransform, RectTransform secondItemRectTransform, Item firstItem, Item secondItem)
	{
			...
			if (newSlotChildCount != 0 && isOnSlot)
			{
				...
				else
				{
					...
					else if (!fitsIntoStack && rest == 0)
					{
						...
						if (oldSlot.transform.parent.parent.GetComponent<EquipmentSystem>() != null && firstItem.itemType == secondItem.itemType)
						{
							newSlot.transform.parent.parent.parent.parent.GetComponent<Inventory>().UnEquipItem1(null, firstItem);
							oldSlot.transform.parent.parent.GetComponent<Inventory>().EquiptItem(null, secondItem);
	...
	private void MoveToEquipment(Transform newSlot, GameObject firstItemGameObject, GameObject secondItemGameObject, RectTransform firstItemRectTransform, RectTransform secondItemRectTransform, Item firstItem, Item secondItem, Inventory inventory)
	{
		//get some informations about the two items
		bool sameItemRerferenced = firstItem.Equals(secondItem);
		...
		if (newSlotChildCount != 0 && isOnSlot)
		{
			...
			if (sameItemType && !sameItemRerferenced) //
			{
				...
				{
					var partSlot = index >= 0 ? eS.hardpointOfSlots[index] : null;
					inventory.GetComponent<Inventory>().EquiptItem(partSlot, firstItem);
			...
			for (int i = 0; i < newSlot.parent.childCount; i++)
			{
				if (newSlot.Equals(newSlot.parent.GetChild(i)))
				{
					...
					if (itemTypeOfSlots[i] == transform.GetComponent<ItemOnObject>().item.itemType)
					{
						...
						// equip slot to equip slot
						if (oldSlot.transform.parent.parent.Equals(newSlot.transform.parent.parent))
						{
							//find old slot index
							var index = -1;
							for (int j = 0; j < oldSlot.transform.parent.childCount; j++)
							{
								if (oldSlot.transform.Equals(oldSlot.transform.parent.GetChild(j)))
								{
									index = j;
								}
							}
							...
							var partSlot = index >= 0 ? eS.hardpointOfSlots[index] : null;
							inventory.GetComponent<Inventory>().UnEquipItem1(partSlot, firstItem);
						}

						inventory.GetComponent<Inventory>().EquiptItem(eS.hardpointOfSlots[i], firstItem);
					}
	...
	private void DropItem()
	{
		...
		if (oldSlot.transform.parent.parent.GetComponent<EquipmentSystem>() != null)
		{
			...
			inventory.GetComponent<Inventory>().UnEquipItem1(partSlot, dropItem.GetComponent<PickUpItem>().item);
		}
